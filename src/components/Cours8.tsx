...
export default function Cours8() {
  ...
  // Localized data arrays (depend on n())
  const ACCORDS_COMMUNS_CG = [
    { accord:"C",  enC:n("pivotAccordCEnC"),  enG:n("pivotAccordCEnG"),  pivot:false, note:n("pivotAccordCGNote") },
    { accord:"Em", enC:n("pivotAccordEmEnC"), enG:n("pivotAccordEmEnG"), pivot:true,  note:n("pivotAccordEmNote") },
    { accord:"Am", enC:n("pivotAccordAmEnC"), enG:n("pivotAccordAmEnG"), pivot:true,  note:n("pivotAccordAmNote") },
    { accord:"G",  enC:n("pivotAccordGEnC"),  enG:n("pivotAccordGEnG"),  pivot:false, note:n("pivotAccordGNote") },
  ];

  const LOGIQUE_STEPS = [
    { n:"1", t:n("logiqueStep1Title"), d:n("logiqueStep1Desc"), c:"#993C1D", bg:"#FAECE7" },
    { n:"2", t:n("logiqueStep2Title"), d:n("logiqueStep2Desc"), c:"#0F6E56", bg:"#E1F5EE" },
    { n:"3", t:n("logiqueStep3Title"), d:n("logiqueStep3Desc"), c:"#185FA5", bg:"#E6F1FB" },
  ];

  const LOGIQUE_TABLE_ROWS = [
    { dest:n("logiqueDestGMaj"), rel:n("logiqueRelDom"),    notes:"6/7", fac:n("logiqueFacTresFacile") },
    { dest:n("logiqueDestFMaj"), rel:n("logiqueRelSousDom"),notes:"6/7", fac:n("logiqueFacTresFacile") },
    { dest:n("logiqueDestAMin"), rel:n("logiqueRelRelMin"), notes:"7/7", fac:n("logiqueFacNaturelle") },
    { dest:n("logiqueDestDMin"), rel:n("logiqueRelII"),     notes:"6/7", fac:n("logiqueFacFacile") },
    { dest:n("logiqueDestEMin"), rel:n("logiqueRelIII"),    notes:"6/7", fac:n("logiqueFacFacile") },
    { dest:n("logiqueDestEbMaj"),rel:n("logiqueRelMed"),    notes:"2-3/7",fac:n("logiqueFacDifficile") },
  ];

  const OUTILS_TABLE_ROWS = [
    { o:"I6/4",      s:n("outilsRow1Struct"), t:n("outilsRow1Tension"), u:n("outilsRow1Usage") },
    { o:"V7sus4",    s:n("outilsRow2Struct"), t:n("outilsRow2Tension"), u:n("outilsRow2Usage") },
    { o:"I6/4+sus4", s:n("outilsRow3Struct"), t:n("outilsRow3Tension"), u:n("outilsRow3Usage") },
  ];
    ...
    <div style={S.wrap}>
      ...
      {sec === "logique" && (
        <div>
          ...
          <p style={S.p} dangerouslySetInnerHTML={{ __html: n("logiqueIntro") }} />

          <div style={S.info} dangerouslySetInnerHTML={{ __html: n("logiqueInfoBox") }} />

          <h3 style={{ fontSize:14, fontWeight:500, margin:"20px 0 8px", color:"#111" }}>{n("logiqueConditionsH3")}</h3>
          <div style={{ display:"flex", flexDirection:"column", gap:8, marginBottom:20 }}>
            {LOGIQUE_STEPS.map(step => (
          ...
          <h3 style={{ fontSize:14, fontWeight:500, margin:"20px 0 8px", color:"#111" }}>{n("logiqueTonsVoisinsH3")}</h3>
          <p style={S.p}>{n("logiqueTonsVoisinsP")}</p>

          <div style={{ overflowX:"auto" }}>
            <table style={{ width:"100%", borderCollapse:"collapse", fontSize:13 }}>
              <thead>
                <tr style={{ borderBottom:"0.5px solid #e5e5e5" }}>
                  {[n("logiqueTableHDest"), n("logiqueTableHRel"), n("logiqueTableHNotes"), n("logiqueTableHFac")].map(h => (
                    <th key={h} style={{ textAlign:"left", padding:"6px 10px", fontWeight:500, color:"#666" }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {LOGIQUE_TABLE_ROWS.map((row, i) => (
                  <tr key={row.dest} style={{ borderBottom:"0.5px solid #f0f0f0", background:i%2===0?"#fff":"#fafafa" }}>
                    ...
                    <td style={{ padding:"7px 10px", color:row.fac===n("logiqueFacTresFacile")||row.fac===n("logiqueFacNaturelle")?"#0F6E56":row.fac===n("logiqueFacFacile")?"#BA7517":"#993C1D" }}>{row.fac}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {sec === "pivot" && (
        <div>
          ...
          <p style={S.p} dangerouslySetInnerHTML={{ __html: n("pivotIntro") }} />

          <div style={S.tip} dangerouslySetInnerHTML={{ __html: n("pivotTipBox") }} />

          <h3 style={{ fontSize:14, fontWeight:500, margin:"20px 0 8px", color:"#111" }}>{n("pivotAccordsH3")}</h3>
          <p style={{ fontSize:13, color:"#888", marginBottom:12 }}>{n("pivotClickHint")}</p>
            ...
            {ACCORDS_COMMUNS_CG.map((ac, i) => (
              <div key={ac.accord} onClick={() => setSelAc(selAc===i?null:i)}
                ...
                <div style={{ fontSize:10, padding:"2px 6px", borderRadius:8, display:"inline-block", background:ac.pivot?"#E1F5EE":"#FAECE7", color:ac.pivot?"#0F6E56":"#993C1D", fontWeight:500 }}>
                  {ac.pivot ? n("pivotGoodLabel") : n("pivotBadLabel")}
                </div>
              </div>
            ))}
          </div>
          ...
          {selAc !== null && (
            <div style={{ border:`0.5px solid ${ACCORDS_COMMUNS_CG[selAc].pivot?"#0F6E56":"#BA7517"}`, borderRadius:10, padding:"14px 16px", background:ACCORDS_COMMUNS_CG[selAc].pivot?"#E1F5EE":"#FAEEDA", marginBottom:16 }}>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:10 }}>
                <div>
                  <div style={{ fontSize:11, color:"#999", marginBottom:3 }}>{n("pivotDetailLabelC")}</div>
                  <div style={{ fontSize:13, fontWeight:500, color:"#111" }}>{ACCORDS_COMMUNS_CG[selAc].enC}</div>
                </div>
                <div>
                  <div style={{ fontSize:11, color:"#999", marginBottom:3 }}>{n("pivotDetailLabelG")}</div>
          ...
          <div style={S.warn} dangerouslySetInnerHTML={{ __html: n("pivotWarnBox") }} />

          <h3 style={{ fontSize:14, fontWeight:500, margin:"20px 0 8px", color:"#111" }}>{n("pivotSchemaH3")}</h3>
          <div style={{ background:"#fafafa", border:"0.5px solid #e5e5e5", borderRadius:10, padding:"14px 16px", marginBottom:12 }}>
            <div style={{ fontSize:12, color:"#999", marginBottom:8 }}>{n("pivotSchemaAnnotation")}</div>
      ...
      {sec === "outils" && (
        <div>
          <h2 style={S.h2}>{n("outilsH2")}</h2>
          <p style={S.p} dangerouslySetInnerHTML={{ __html: n("outilsIntro") }} />

          {/* 6/4 */}
          <div style={{ border:"0.5px solid #185FA530", borderRadius:10, padding:"16px", background:"#E6F1FB", marginBottom:16 }}>
            <h3 style={{ fontSize:15, fontWeight:500, color:"#185FA5", marginBottom:8 }}>{n("outilsQuarteSixteH3")}</h3>
            <p style={{ fontSize:13, color:"#0C447C", lineHeight:1.65, marginBottom:12 }} dangerouslySetInnerHTML={{ __html: n("outilsQuarteSixteP") }} />
            <div style={{ background:"rgba(255,255,255,0.7)", borderRadius:8, padding:"10px 14px", marginBottom:12 }}>
              <div style={{ fontSize:12, color:"#999", marginBottom:6 }}>{n("outilsResolutionLabel64")}</div>
            ...
            <div style={{ fontSize:12, color:"#888", marginTop:10 }}>{n("outilsDoublureNote")}</div>
          </div>

          {/* V7sus4 */}
          <div style={{ border:"0.5px solid #534AB730", borderRadius:10, padding:"16px", background:"#EEEDFE", marginBottom:16 }}>
            <h3 style={{ fontSize:15, fontWeight:500, color:"#534AB7", marginBottom:8 }}>{n("outilsSusH3")}</h3>
            <p style={{ fontSize:13, color:"#2E2A87", lineHeight:1.65, marginBottom:12 }} dangerouslySetInnerHTML={{ __html: n("outilsSusP") }} />
            <div style={{ background:"rgba(255,255,255,0.7)", borderRadius:8, padding:"10px 14px", marginBottom:12 }}>
              <div style={{ fontSize:12, color:"#999", marginBottom:6 }}>{n("outilsResolutionLabelSus")}</div>
            ...
            <button onClick={() => playProg(ref as React.RefObject<PianoPlayerRef>, ["G7sus4","G7","C"], 950)}
              ...
              {n("outilsListenBtnSus")}
            </button>
          </div>

          <div style={S.tip} dangerouslySetInnerHTML={{ __html: n("outilsTipBox") }} />

          <h3 style={{ fontSize:14, fontWeight:500, margin:"20px 0 8px", color:"#111" }}>{n("outilsComparaisonH3")}</h3>
          <div style={{ overflowX:"auto" }}>
            <table style={{ width:"100%", borderCollapse:"collapse", fontSize:13 }}>
              <thead>
                <tr style={{ borderBottom:"0.5px solid #e5e5e5" }}>
                  {[n("outilsTableHOutil"), n("outilsTableHStructure"), n("outilsTableHTension"), n("outilsTableHUsage")].map(h => (
                    <th key={h} style={{ textAlign:"left", padding:"6px 10px", fontWeight:500, color:"#666" }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {OUTILS_TABLE_ROWS.map((row, i) => (
      ...
      {sec === "exemples" && (
        <div>
          ...
          <h3 style={{ fontSize:14, fontWeight:500, margin:"24px 0 8px", color:"#111" }}>{n("exemplesModCompleteH3")}</h3>
          <div style={{ background:"#fafafa", border:"0.5px solid #e5e5e5", borderRadius:10, padding:"14px 16px", marginBottom:12 }}>
            <div style={{ fontSize:12, color:"#999", marginBottom:6 }}>{n("exemplesModCompleteAnnotation")}</div>
            ...
            <div style={{ fontSize:12, color:"#666", lineHeight:1.65, marginBottom:10 }}>
              {n("exemplesModCompleteFootnote")}
            </div>
            <button
              onClick={() => playProg(ref as React.RefObject<PianoPlayerRef>, ["C","F/A","G7/B","C","Am_pivot","D7/C","C/G","G7/B","G"], 950)}
              style={{ fontSize:12, padding:"5px 14px", border:"0.5px solid #185FA5", borderRadius:20, cursor:"pointer", background:"transparent", color:"#185FA5" }}>
              {n("exemplesModCompleteListenBtn")}
            </button>
          </div>

          <div style={S.info} dangerouslySetInnerHTML={{ __html: n("exemplesInfoBox") }} />
        </div>
      )}
      ...
      {sec !== "quiz" && (
        <div style={{ marginTop: 32 }}>
          ...
          {INVERSION_EXERCISES.filter(e => e.cours === 8).map(ex => (
