...
import { getInstrument, playChordNow } from '@/components/PianoPlayer';
...
export default function StaffNotation({
  ...
  useEffect(() => {
    ...
    let mounted = true;
    // Réutilise le moteur audio singleton partagé (@tonejs/piano)
    getInstrument()
      .then(() => { if (mounted) setAudioReady(true); })
      .catch(() => { if (mounted) setAudioReady(true); });
  ...
  const playChord = async (idx: number) => {
    ...
    await getInstrument();
    playChordNow(CHORDS[idx].midi, '2n');
  };
