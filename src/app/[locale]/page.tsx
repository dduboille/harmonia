...
const FEATURES = [
  { icon: "𝄞", title: "9 cours de théorie", desc: "Des origines acoustiques de la gamme aux modulations avancées — une progression pédagogique rigoureuse, inspirée des meilleures méthodes académiques." },
  ...
  { icon: "🔊", title: "Audio Salamander", desc: "Piano à queue Steinway samplé en haute fidélité. Écoutez chaque accord, chaque progression, chaque voicing que vous construisez." },
...
export default function LandingPage() {
  ...
  return (
    <div style={{ fontFamily: "'Georgia', 'Times New Roman', serif", background: "#faf8f4", color: "#1a1a1a", overflowX: "hidden" }}>
      ...
      {/* Navigation */}
      <nav style={{ position: "fixed" as const, top: 0, left: 0, right: 0, zIndex: 100, background: "rgba(250,248,244,0.95)", backdropFilter: "blur(8px)", borderBottom: "0.5px solid #e8e3db", padding: "0 2rem", height: 60, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
      ...
      {/* Hero */}
      <section style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: "120px 2rem 80px", position: "relative" as const, overflow: "hidden" }}>
        <div style={{ position: "absolute" as const, top: "10%", right: "-5%", fontSize: 320, color: "#BA7517", opacity: 0.04, fontFamily: "serif", userSelect: "none" as const, lineHeight: 1 }}>𝄞</div>
        <div style={{ position: "absolute" as const, bottom: "5%", left: "-3%", fontSize: 240, color: "#185FA5", opacity: 0.04, fontFamily: "serif", userSelect: "none" as const, lineHeight: 1 }}>𝄢</div>
        {["Dm7", "G7", "CMaj7", "Am", "F", "Bdim"].map((chord, i) => (
          <div key={chord} style={{ position: "absolute" as const, fontSize: 13, fontFamily: "monospace", color: i % 2 === 0 ? "#BA7517" : "#185FA5", opacity: 0.12, top: `${15 + i * 12}%`, left: i < 3 ? `${3 + i * 4}%` : `${75 + (i - 3) * 7}%`, fontWeight: 700 }}>{chord}</div>
        ))}
        ...
        <div style={{ maxWidth: 760, textAlign: "center" as const, position: "relative" as const, zIndex: 1 }}>
          ...
          <p style={{ fontSize: "clamp(16px, 2.5vw, 20px)", color: "#666", lineHeight: 1.7, maxWidth: 560, margin: "0 auto 40px", fontFamily: "system-ui, sans-serif", fontWeight: 400 }}>
            De la gamme aux modulations avancées — 9 cours interactifs, 700+ exercices avec feedback harmonique en temps réel, dans les 24 tonalités.
          </p>

          {/* Partition Pathétique */}
          <div style={{ margin: "0 auto 40px", maxWidth: 700, borderRadius: 12, overflow: "hidden", boxShadow: "0 4px 24px rgba(0,0,0,0.15)" }}>
            <img
              src="/partition-beethoven.jpg"
              alt="Sonate Pathétique de Beethoven avec analyse harmonique"
          ...
          <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" as const }}>
            ...
            <Link href={`/${locale}/tonalites`} style={{ padding: "14px 32px", borderRadius: 4, background: "transparent", color: "#1a1a1a", textDecoration: "none", fontSize: 15, fontFamily: "system-ui, sans-serif", border: "1px solid #c8c4bc" }}>
