# Cursus Conservatoire — Plan d'implémentation

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Créer la page vitrine `/[locale]/cursus` + l'API Resend associée, et l'intégrer dans la navigation existante.

**Architecture:** Page `"use client"` monolithique (~700 lignes, même pattern que la landing page), avec `Set<number>` pour l'accordion multi-open, et un div+onClick pour le formulaire (pas de `<form>`). API route copiée sur le pattern `/api/contact-conservatoire/route.ts`.

**Tech Stack:** Next.js (App Router), React, TypeScript, Resend, styles 100% inline

> **Note AGENTS.md :** Avant d'écrire du code, lire les guides pertinents dans `node_modules/next/dist/docs/` (App Router, server vs client components). Les patterns utilisés ici sont calqués sur l'existant (`"use client"` + `useParams()` comme dans `src/app/[locale]/page.tsx`).
...
| Action | Fichier |
|--------|---------|
| Create | `src/app/api/contact-cursus/route.ts` |
| Create | `src/app/[locale]/cursus/page.tsx` |
| Modify | `src/components/AppNav.tsx` |
| Modify | `src/app/[locale]/page.tsx` |

---

## Task 1 — API route `/api/contact-cursus/route.ts`

**Files:**
- Create: `src/app/api/contact-cursus/route.ts`

- [ ] **Step 1.1 : Créer le répertoire et le fichier**

```bash
mkdir -p src/app/api/contact-cursus
```

Créer `src/app/api/contact-cursus/route.ts` avec le contenu complet suivant :
...
```bash
git add src/app/api/contact-cursus/route.ts
git commit -m "feat: API route contact-cursus — Resend double email"
```

---

## Task 2 — Page `/[locale]/cursus/page.tsx`
...
```bash
mkdir -p "src/app/[locale]/cursus"
```

- [ ] **Step 2.2 : Créer `src/app/[locale]/cursus/page.tsx`**
      ...
      {open && (
        <div style={{ padding: "28px", borderTop: "0.5px solid #e8e3db", background: "#faf8f4" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24, marginBottom: 28 }}>
            <div>
              <div style={{ fontSize: 10, fontWeight: 700, color: "#BA7517", letterSpacing: "0.12em", marginBottom: 8, fontFamily: "system-ui" }}>
                OBJECTIF
              </div>
              <p style={{ fontSize: 14, color: "#444", lineHeight: 1.7, margin: 0, fontFamily: "system-ui, sans-serif" }}>
                {level.goal}
              </p>
            </div>
            <div>
          ...
          <div>
            <div style={{ fontSize: 10, fontWeight: 700, color: "#BA7517", letterSpacing: "0.12em", marginBottom: 14, fontFamily: "system-ui" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 8 }}>
              {level.modules.map((m, i) => (
                ...
                >
                  <span style={{ color: "#BA7517", flexShrink: 0, fontWeight: 700, marginTop: 1 }}>→</span>
                  {m}
                </div>
              ))}
            </div>
    ...
    <div style={{
      ...
      <section style={{ padding: "100px 2rem 80px", textAlign: "center" as const }}>
        <div style={{ maxWidth: 760, margin: "0 auto" }}>
          ...
          <h1 style={{
            ...
            Un cursus d'harmonie de niveau<br />
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 2 }}>
            {PHILO.map((p, i) => (
              ...
              >
                ...
                <p style={{ fontSize: 14, color: "#666", lineHeight: 1.75, margin: 0, fontFamily: "system-ui, sans-serif" }}>
                  {p.desc}
                </p>
              </div>
            ))}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 16 }}>
            {REFS_DATA.map(ref => (
              ...
              >
                ...
                <p style={{ fontSize: 13, color: "#666", lineHeight: 1.65, margin: 0, fontFamily: "system-ui, sans-serif" }}>
                  {ref.usage}
                </p>
              </div>
            ))}
        ...
        <div style={{ maxWidth: 780, margin: "0 auto" }}>
          ...
          <div style={{ border: "0.5px solid #e8e3db", borderRadius: 8, overflow: "hidden" }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", background: "#1a1a1a" }}>
              <div style={{ padding: "16px 20px", fontSize: 11, fontWeight: 700, color: "#555", fontFamily: "system-ui", letterSpacing: "0.1em" }} />
              <div style={{ padding: "16px 20px", fontSize: 11, fontWeight: 700, color: "#777", fontFamily: "system-ui", letterSpacing: "0.1em", borderLeft: "0.5px solid #333" }}>
                COURS INTERNET
              </div>
              <div style={{ padding: "16px 20px", fontSize: 11, fontWeight: 700, color: "#E9C97E", fontFamily: "system-ui", letterSpacing: "0.1em", borderLeft: "0.5px solid #333" }}>
                HARMONIA CURSUS
              </div>
            </div>
            {COMPARE_ROWS.map((row, i) => (
              ...
              >
                <div style={{ padding: "16px 20px", fontSize: 13, fontWeight: 600, color: "#444", fontFamily: "system-ui" }}>
                  {row.label}
                </div>
                <div style={{ padding: "16px 20px", fontSize: 13, color: "#aaa", fontFamily: "system-ui, sans-serif", borderLeft: "0.5px solid #e8e3db" }}>
                  {row.internet}
                </div>
                <div style={{ padding: "16px 20px", fontSize: 13, color: "#1a1a1a", fontFamily: "system-ui, sans-serif", fontWeight: 500, borderLeft: "0.5px solid #e8e3db" }}>
                  <span style={{ color: "#BA7517", marginRight: 6 }}>✓</span>{row.harmonia}
                </div>
              </div>
            ))}
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
            {ETABLISSEMENTS.map((e, i) => (
              ...
              >
                ...
                <h3 style={{ fontSize: 16, fontWeight: 500, margin: "0 0 12px", lineHeight: 1.3, letterSpacing: "-0.01em" }}>
                  {e.title}
                </h3>
                <p style={{ fontSize: 14, color: "#666", lineHeight: 1.7, margin: 0, fontFamily: "system-ui, sans-serif" }}>
                  {e.desc}
                </p>
              </div>
            ))}
      ...
      <section style={{ padding: "100px 2rem" }}>
        <div style={{ maxWidth: 860, margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16, alignItems: "start", marginBottom: 24 }}>
            {PRICING.map(plan => (
              <div
                key={plan.label}
                style={{
            ...
            <div style={{ display: "flex", flexDirection: "column" as const, gap: 16 }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                <div>
                  ...
                  <input value={form.prenom} onChange={set("prenom")} style={INPUT} placeholder="Sophie" />
                </div>
                <div>
                  <label style={LABEL}>Nom *</label>
                  <input value={form.nom} onChange={set("nom")} style={INPUT} placeholder="Martin" />
                </div>
              </div>
              <div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                <div>
                  ...
                  <input value={form.ville} onChange={set("ville")} style={INPUT} placeholder="Lyon" />
                </div>
                <div>
                  <label style={LABEL}>Pays</label>
                  <input value={form.pays} onChange={set("pays")} style={INPUT} placeholder="France" />
                </div>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                <div>
          ...
          <div style={{ display: "flex", gap: 24, flexWrap: "wrap" as const }}>
            {[
              ...
              <Link
                key={link.label}
                href={link.href}
                style={{ fontSize: 13, color: "#555", textDecoration: "none", fontFamily: "system-ui, sans-serif" }}
              >
                {link.label}
              </Link>
            ))}
...
```bash
git add "src/app/[locale]/cursus/page.tsx"
git commit -m "feat: page vitrine /cursus — accordion 5 niveaux + formulaire"
```

---

## Task 3 — Modifier `AppNav.tsx`

**Files:**
- Modify: `src/components/AppNav.tsx:202`

- [ ] **Step 3.1 : Ajouter le NavItem Cursus**

Dans `src/components/AppNav.tsx`, trouver la ligne du NavItem `comparateur` (environ ligne 202) :
...
git commit -m "feat: ajout lien Cursus dans AppNav"
```

---

## Task 4 — Modifier `src/app/[locale]/page.tsx` (landing nav)

**Files:**
- Modify: `src/app/[locale]/page.tsx:169`

- [ ] **Step 4.1 : Ajouter le lien Cursus dans le nav landing**

Dans `src/app/[locale]/page.tsx`, trouver le bloc nav fixe (~ligne 167-170) :
...
- [ ] **Step 4.2 : Ajouter le lien Cursus dans le footer de la landing**

Dans le footer de `src/app/[locale]/page.tsx` (~ligne 425-431), trouver le tableau de liens :
...
git commit -m "feat: lien Cursus dans nav et footer landing page"
```
...
- [ ] **Step 5.1 : Build complet**

```bash
npm run build
```

Attendu : `✓ Compiled successfully`, pas d'erreur de type ni de lint.

Si erreur de type : corriger dans le fichier concerné, relancer `npx tsc --noEmit`.

- [ ] **Step 5.2 : Vérification manuelle en dev**

```bash
npm run dev
```

Ouvrir `http://localhost:3000/fr/cursus` et vérifier :
- [ ] Hero visible, badge amber, deux CTAs
- [ ] Les 5 accordions s'ouvrent/ferment indépendamment (cliquer niveau 2, puis niveau 4 — les deux restent ouverts)
- [ ] L'ancre `#contact` scrolle vers le formulaire
- [ ] L'ancre `#programme` scrolle vers les niveaux
- [ ] Le formulaire sans `<form>` : remplir les champs obligatoires, cliquer "Envoyer" → spinner → message de succès
- [ ] AppNav affiche "🎓 CURSUS" et le lien est actif sur `/fr/cursus`
- [ ] Landing page (`/fr`) affiche "Cursus" entre "Tonalités" et le sélecteur de langue
...
Vérifier sur `https://www.getharmonia.app/fr/cursus`.
